...
// Vendored from framework-arduinoespressif32's libraries/Update/src/Updater.cpp
// to patch _writeBuffer()'s erase behavior for BLE OTA reliability on the
// ESP32-C3 -- see the comment in _writeBuffer() below. This project's lib/
// folder takes precedence over the framework's bundled Update library.
...
bool UpdateClass::_writeBuffer(){
    ...
    if (block_erase || part_head_sectors || part_tail_sectors){
        // Vendored from the stock Updater.cpp: erase one 4K sector at a time
        // instead of the whole 64K block in a single call. A 64K block erase
        // holds flash cache/interrupts disabled long enough (several hundred
        // ms+) to trip the fixed 300ms ESP_INT_WDT and hard-reset the chip
        // mid-OTA -- which looks like a BLE disconnect but is actually a
        // panic reboot. Sector erases stay well under that ceiling, and the
        // yield() between them lets the BLE stack service the link.
        const size_t eraseLen = block_erase ? SPI_FLASH_BLOCK_SIZE : SPI_FLASH_SEC_SIZE;
        for (size_t done = 0; done < eraseLen; done += SPI_FLASH_SEC_SIZE) {
            if(!ESP.partitionEraseRange(_partition, _progress + done, SPI_FLASH_SEC_SIZE)){
